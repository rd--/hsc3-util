...
import qualified Sound.SC3.UGen.Dot as D {- hsc3-dot -}
...
scsyndef_to_dot opt sy_nm = do
  ...
  let dot_nm = replaceExtension sy_nm "dot"
      ...
      dr = D.dotGraph (if opt then D.svg_options else D.dot_options) gr'
  writeFile dot_nm dr
...
main :: IO ()
main = do
  let usage = putStrLn "scsyndef-to-dot [-svg] file..."
  arg <- getArgs
  case arg of
    [] -> usage
    ["-h"] -> usage
